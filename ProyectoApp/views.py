...
from .models import CustomUser, AporteAfiliado, AporteMensual, AporteCertificacion
				...
				if form.cleaned_data['password1'] == form.cleaned_data['password2']:
					...
					try:
						...
						user_new = CustomUser.objects.create_user(
							...
							num_ruc = form.cleaned_data['num_ruc'],
							...
							ejer_cated = form.cleaned_data['ejer_cated'],
	...
	def get(self, request, user_log, *args, **kwargs):
		if request.user.groups.filter(name__in=['Directivo']):
			...
			form = FormularioModificarDatos(
				...
				'ejer_cated': user_log.ejer_cated,
		...
		if request.user.groups.filter(name__in=['Directivo']):
			...
			if form.is_valid():
				...
				try:	
					...
					user_mod.num_ruc = form.cleaned_data['num_ruc']
					...
					user_mod.ejer_cated = form.cleaned_data['ejer_cated']
	...
	def get(self, request, user_log, user_filt, *args, **kwargs):
		if request.user.groups.filter(name__in=['Directivo']):
			...
			form = FormularioActualizarDatos(
				...
				'ejer_cated': user_filt.ejer_cated,
		...
		if request.user.groups.filter(name__in=['Recaudador']):
			...
			if form.is_valid():
				...
				return HttpResponseRedirect(
						f'/recaudadorcobrarconfdatos/{user_log}/{usuario}{ta}/{talon}/{fecha_pago}/{etiq}/'
					)
...
class RecaudadorCobroConfirmarDatos(DetailView):
	def get(self, request, user_log, usuario, ta, talon, fecha_pago, etiq, *args, **kwargs):
		if request.user.groups.filter(name__in=['Recaudador']):
	...
	def post(self, request, user_log, usuario, ta, talon, fecha_pago, etiq, *args, **kwargs):
	
		if request.user.groups.filter(name__in=['Recaudador']):
			context = cobrar_aporte_confirmado(request, user_log, usuario, ta, talon, fecha_pago, etiq)
	...
	def post(self, request, user_log, usuario, *args, **kwargs):
		if request.user.groups.filter(name__in=['Recaudador']):
			...
			if form.is_valid():
				...
				messages.info(request, 'Revise los datos y proceda a ejecutar...')
				return HttpResponseRedirect(f'/recaudadorcobromultefect/{user_log}/{usuario}/{fecha_pago}/{cant}/{tam}/{talon}/')
...
class RecaudadorCobroMultipleEfect(DetailView):
	def get(self, request, user_log, usuario, fecha_pago, cant, tam, talon):
		if request.user.groups.filter(name__in=['Recaudador']):
	...
	def post(self, request, user_log, usuario, fecha_pago, cant, tam, talon):
		
		context = cobrar_aporte_mult(request, user_log, usuario, fecha_pago, cant, tam, talon)
